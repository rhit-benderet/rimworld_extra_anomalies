...
{
	public class JobDriver_PutMonitorOnLeg : JobDriver
	{
		private ThingWithComps SyntheticLeg
		{
			get
			{
		...
		{
			return this.pawn.Reserve(this.SyntheticLeg, this.job, 1, -1, null, errorOnFailed, false) && this.pawn.Reserve(this.Item, this.job, 1, -1, null, errorOnFailed, false);
		}
		...
		protected override IEnumerable<Toil> MakeNewToils()
		{
			...
			toil.tickAction = delegate()
			{
				...
				{
					this.warmupMote = MoteMaker.MakeAttachedOverlay(this.SyntheticLeg, compUsable.Props.warmupMote, Vector3.zero, 1f, -1f);
				}
		...
		private void Attach()
		{
			Comp_SyntheticLeg legComp = this.SyntheticLeg.TryGetComp<Comp_SyntheticLeg>();
			if (legComp != null)
			{
				
				Map map = this.SyntheticLeg.Map;
        		IntVec3 position = this.SyntheticLeg.Position;
				Thing thing = ThingMaker.MakeThing(legComp.Props.turnIntoOnceAttached);
				this.Item.SplitOff(1).Destroy(DestroyMode.Vanish);
				this.SyntheticLeg.SplitOff(1).Destroy(DestroyMode.Vanish);
